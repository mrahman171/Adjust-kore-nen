...
export function ScoreModal({ score, breakdown, onContinue, onEnd }: Props) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 py-8">
      <section className="w-full max-w-lg rounded-3xl border-2 border-[var(--accent)] bg-[var(--paper)] p-6 text-center shadow-[0_25px_70px_rgba(140,40,30,0.25)] sm:p-8 max-h-[85vh] overflow-y-auto">
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-center">
          <button
            className="w-full rounded-full border-2 border-[var(--accent)] px-6 py-3 text-xs font-semibold uppercase tracking-[0.2em] text-[var(--accent)] transition hover:bg-[var(--accent)] hover:text-white sm:w-auto"
            onClick={onContinue}
          >
            চালিয়ে যান
          </button>
